...
def get_data(city):
    ...
    url = 'https://api3-core-c-lq.amemv.com/aweme/v1/nearby/feed/?max_cursor=0&min_cursor=0&count=20&feed_style=1' \
          ...
          '=100901&dpi=240&uuid=355757648741243&app_name=aweme&version_name=10.9.0&ts=1589104292&app_type=normal&ac' \
          ...
          '&language=zh&device_brand=samsung&aid=1128&mcc_mnc=46007'.format(city_id, city_name)
    try:
...
def result_parse(result):
    ...
    try:
        for aweme in result:
            ...
            title = aweme['desc']
            video_url = aweme['video']['download_suffix_logo_addr']['url_list'][0]
            author_sec_uid = aweme['author']['sec_uid']
            production_dict = {
                'create_time': fabu_time(create_time),
...
if __name__ == '__main__':
    city = {'city_id': 620500, 'city_name': '天水'}
    result = get_data(city)
    # print(result)
    data = result_parse(result)
    print(data)
